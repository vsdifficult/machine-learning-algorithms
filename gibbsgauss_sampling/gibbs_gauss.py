    ...
    def sample(self, mu, Sigma, xinit, num_samples): 
        ...
        for s in range(num_samples): 
            for d in range(dim): 
                ...
                x[d] = np.random.normal(mu_AgivenB[0, 0], np.sqrt(Sigma_AgivenB[0, 0])) 
            samples[s, :] = x.flatten() 
...
if __name__ == "__main__": 
    ...
    plt.figure() 
    plt.scatter(gibbs_samples[:, 0], gibbs_samples[:, 1], alpha=0.5, label='Gibbs Samples') 
    plt.scatter(scipy_samples[:, 0], scipy_samples[:, 1], alpha=0.5, label='SciPy Samples') 
    plt.xlabel('X1') 
    plt.ylabel('X2') 
    plt.legend() 
    plt.title('Gibbs Sampling vs SciPy Sampling') 
    plt.show()
